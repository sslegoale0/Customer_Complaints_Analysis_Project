...
FROM complaints
WHERE [Progress] = 'Closed')*100/(COUNT(DISTINCT [Complaint ID])), 2) AS "% of Total Closed Complaints"
FROM complaints;
...
ROUND((SELECT CAST(COUNT(DISTINCT [Complaint ID]) AS FLOAT)
FROM complaints
WHERE [Progress] = 'Pending')*100/(COUNT(DISTINCT [Complaint ID])), 2) AS "% of Total Pending Complaints"
FROM complaints;
...
ROUND((SELECT CAST(COUNT(DISTINCT [Complaint ID]) AS FLOAT)
FROM complaints
WHERE [Timely response?] = 'Yes')*100/(COUNT(DISTINCT [Complaint ID])), 2) AS "% of Total Timely Responded Complaints"
FROM complaints;
...
ROUND((SELECT CAST(COUNT(DISTINCT [Complaint ID]) AS FLOAT)
FROM complaints
WHERE [Consumer disputed?] = 'Yes')*100/(COUNT(DISTINCT [Complaint ID])), 2) AS "% of Total Consumer Disputed Complaints"
FROM complaints;
